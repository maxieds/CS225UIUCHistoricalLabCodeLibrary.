...
vector<Etype>::iterator::iterator() : ptr(NULL)
{
...
template <typename Etype>
Etype & vector<Etype>::iterator::operator*() 
{ 
   return *ptr;  
}
...
template <typename Etype>
typename vector<Etype>::iterator 
                     vector<Etype>::iterator::operator++(int ignore) 
{
...
template <typename Etype>
bool vector<Etype>::iterator::operator==(
                 typename vector<Etype>::iterator const & origVal) const
{ 
...
template <typename Etype>
vector<Etype>::iterator::iterator(Etype * assignPtr) : ptr(assignPtr)
{ 
   // initializer list used above; no code needed here
}
...
template <typename Etype>
vector<Etype>::const_iterator::const_iterator() : ptr(NULL)
{
   // initializer list used above; no code needed here
}
...
vector<Etype>::const_iterator::const_iterator(
        typename vector<Etype>::iterator const & theIter) : ptr(theIter.ptr)
{
   // initializer list used above; no code needed here
}
...
template <typename Etype>
Etype const & vector<Etype>::const_iterator::operator*() const
{ 
   return *ptr;  
}
...
template <typename Etype>
bool vector<Etype>::const_iterator::operator==(
...
//          a legal cell on which to perform this operation.
template <typename Etype>
Etype & vector<Etype>::reverse_iterator::operator*()
{
...
//         iterator point to the same location; else returns false
template <typename Etype>
bool vector<Etype>::reverse_iterator::operator==(
...
vector<Etype>::reverse_iterator::reverse_iterator(
          typename vector<Etype>::iterator assignIter) : current(assignIter)
{
   // initializer list used above; no code needed here
}
...
//         which to perform this operation.
template <typename Etype>
Etype const & vector<Etype>::const_reverse_iterator::operator*() const
{
   iterator temp = current;
   --temp;
   return *temp;
...
{
   typename vector<Etype>::const_reverse_iterator temp = *this;
   --current;
   return temp;
}
...
//         iterator point to the same location; else returns false
template <typename Etype>
bool vector<Etype>::const_reverse_iterator::operator==(
...
vector<Etype>::vector(int requestedSize)
{
   ...
   finish = start + requestedSize; // pointer arithmetic, not integer addition
...
template <typename Etype>
vector<Etype>::vector(int requestedSize, Etype const & initElem) 
{
...
template <typename Etype>
vector<Etype>::vector(vector<Etype> const & origVal)
{
   vectorCapacity = origVal.vectorCapacity;
   vectorSize = origVal.vectorSize;
   
   start = new Etype[vectorCapacity];
   finish = start + vectorSize;  // pointer arithmetic, not integer addition
   onePastEnd = start + vectorCapacity;  // pointer arithmetic
...
template <typename Etype>
vector<Etype> const & vector<Etype>::operator=(vector<Etype> const & origVal)
{
   if (this != &origVal)
   {
      delete[] start;
...
//        to this object invalid
template <typename Etype>
void vector<Etype>::assign(int requestedSize, Etype const & assignElem)
{
   delete[] start;
...
{
   return vector<Etype>::iterator(start);   
}
...
typename vector<Etype>::iterator vector<Etype>::end() 
{
   ...
   return vector<Etype>::iterator(finish);
}
...
typename vector<Etype>::reverse_iterator vector<Etype>::rbegin() 
{
   ...
   //   type returned. 
   return vector<Etype>::reverse_iterator(finish);
}
...
typename vector<Etype>::reverse_iterator vector<Etype>::rend() 
{
   ...
   return vector<Etype>::reverse_iterator(start);
}
...
template <typename Etype>
Etype const & vector<Etype>::front() const
{
   return *start;
}
...
template <typename Etype>
Etype const & vector<Etype>::back() const
{
   // pointer arithmetic; finish - one cell, not finish (an 
   //    address value) - one value (which would give the last
   //    byte of the previous cell)
...
//   - no range checking is done here
template <typename Etype>
Etype const & vector<Etype>::operator[] (int index) const
{
   return start[index];
}
...
//    - range checking is done here
template <typename Etype>
Etype const & vector<Etype>::at(int index) const
{
   Assert(((index >=0) && (index < vectorSize)), "Bad array access"); 
   return start[index];
}
...
template <typename Etype>
void vector<Etype>::push_back(Etype const & insElem)
{
...
template <typename Etype>
void vector<Etype>::insert(typename vector<Etype>::iterator insIter, 
                                               Etype const & newElem)
{
   Etype* travPtr;
...
template <typename Etype>
void vector<Etype>::erase(typename vector<Etype>::iterator eraseIter)
{
   typename vector<Etype>::iterator temp = eraseIter;
   typename vector<Etype>::iterator temp2 = eraseIter;
   ++temp2;
   while (temp2 != end())
   {
      *temp = *temp2;
...
template <typename Etype>
void vector<Etype>::erase(typename vector<Etype>::iterator first,
                          typename vector<Etype>::iterator last)
{
   // count how many cells are being erased
   typename vector<Etype>::iterator temp = first;
   int n = 0;
   while (temp != last)
   {
      n++; 
   ...
   temp = first;
   typename vector<Etype>::iterator temp2 = last;
   while (temp2 != end())
   {
      *temp = *temp2;
      temp++;
...
//           else returns false
template <typename Etype>
bool vector<Etype>::operator==(vector<Etype> const & secondVal)
{
   bool result = true;
   if (vectorSize == secondVal.vectorSize)
   {
      typename vector<Etype>::const_iterator temp = begin();
      typename vector<Etype>::const_iterator tempParam = secondVal.begin();
      while (temp != end())
      {
         if (*temp != *tempParam)
         {
...
bool vector<Etype>::operator<(vector<Etype> const & secondVal)
{
   ...
   while ((temp != end()) && (tempParam != secondVal.end()) && stillEqual)
   {
      if (*temp < *tempParam)
      {
         result = true;
   ...
   if (stillEqual)  // ended because one or both at end
   {
      // If vectors are equal up to the point where at
      //   least one of them ends, then the first vector is only
      //   less if it has ended and there are more values
      //   in the second vector. If both have ended, then they
      //   are completely equal, and if the second one ends and
      //   the first one has not, then the second one is less
      //   than the first one.
      if ((temp == end()) && (tempParam != secondVal.end()))
         result = true;
      else
         result = false;
   }
...
template <typename Etype>
std::ostream& operator<<(std::ostream& Out, vector<Etype> const & printvector)
{
   int temp = 0;
