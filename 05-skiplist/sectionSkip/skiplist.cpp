...
Skiplist<Etype>::Skiplist() : head(0, 5), currentPtr(NULL), 
                                maxLevel(5), size(0)
{
...
void Skiplist<Etype>::back()
{
   ...
   else
   {
      ...
      //   sunk below the bottom level of the list) and the header pointer 
      //   is NULL, move down a level. 
...
void Skiplist<Etype>::backwardOne()
{
   if (size > 0)
   {
      if (currentPtr != head[0])
      {
         ...
         travPtr = head[0];
         while ((travPtr->ptrArray)[0] != currentPtr)
            travPtr = (travPtr->ptrArray)[0];
         currentPtr = travPtr;
...
/*
         ...
         // first element. We took care of both cases with conditionals 
         // above.
